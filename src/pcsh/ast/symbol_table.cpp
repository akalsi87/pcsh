...
namespace symbol_table {
    ...
    {
        inline size_t operator()(const ast::variable* v) const
        {
            auto s = v->name();
            size_t hash = 0;
            for (; *s; ++s) {
    ...
    {
        inline bool operator()(const ast::variable* v1, const ast::variable* v2) const
        {
    ...
    class table_impl
      : public std::unordered_map<const ast::variable*, entry, variable_name_hash, variable_name_comp>
    { };
...
namespace detail {
    ...
    void set(const ptr& tbl, const ast::variable* v, ast::node* value, result_type ty, bool eval)
    {
        (*tbl)[v] = { value, ty, eval };
    }
